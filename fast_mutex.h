...
#   if !defined(NVWA_USE_CXX11_MUTEX) && \
...
#   if !defined(_NOTHREADS)    && !defined(NVWA_NOTHREADS) && \
...
private:
    fast_mutex_autolock(const fast_mutex_autolock&) = delete;
    fast_mutex_autolock& operator=(const fast_mutex_autolock&) = delete;
};
} // namespace nvwa
