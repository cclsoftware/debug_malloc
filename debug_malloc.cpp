...
/**
 * @file  debug_malloc.cpp
 *
 * Implementation of debug versions of malloc, realloc and free to check leakage.
 */
...
struct context_t {
    fast_mutex* lock;              ///< Pointer to mutex to protect access to the pointer list
    size_t*     current_mem_alloc; ///< Pointer to total allocated memory amount in bytes
};
...
struct new_ptr_list_t {
    ...
    context_t*      context;    ///< Pointer to allocation context
    union {
    ...
    uint32_t        line;       ///< Line number of the caller; or \c 0
...
/**
 * Context for allocations made by this module.
 */
context_t allocation_context = {
    &new_ptr_lock,
    &current_mem_alloc
};
...
#endif
		auto buf_len = strlen(new_progname)  + exeext_len
...
new_ptr_list_t* convert_user_ptr(void* usr_ptr, size_t alignment)
{
    ...
    if (ptr->magic == DEBUG_NEW_MAGIC && !is_adjusted) {
        return ptr;
    }
...
void* debug_malloc(size_t size, const char* file, int line)
{
    ...
    uint32_t aligned_list_item_size = align(sizeof(new_ptr_list_t), _DEBUG_NEW_ALIGNMENT);
    size_t s = size + aligned_list_item_size + _DEBUG_NEW_TAILCHECK;
    auto ptr = static_cast<new_ptr_list_t*>(debug_new_alloc(s, _DEBUG_NEW_ALIGNMENT));
    if (ptr == nullptr) {
...
#endif
    ptr->context = &allocation_context;
    ptr->size = size;
 ...
 */
void* debug_realloc(void* usr_ptr, size_t size, const char* file, int line)
{
    if (usr_ptr == nullptr)
    ...
    if (ptr == nullptr) {
        {
            fast_mutex_autolock lock(new_output_lock);
            fprintf(new_output_fp, "realloc: invalid pointer %p (",
                usr_ptr);
            print_position(nullptr, 0);
            fprintf(new_output_fp, ")\n");
        }
    ...
    memcpy(new_ptr, usr_ptr, ptr->size);
...
void debug_free(void* usr_ptr)
{
    ...
    if (ptr == nullptr) {
        {
            fast_mutex_autolock lock(new_output_lock);
            fprintf(new_output_fp, "free: invalid pointer %p (",
                    usr_ptr);
            print_position(nullptr, 0);
            fprintf(new_output_fp, ")\n");
        }
...
#endif
    {
        fast_mutex_autolock lock(*ptr->context->lock);
        *ptr->context->current_mem_alloc -= ptr->size;
        ptr->magic = 0;
    ...
    if (new_verbose_flag) {
        fast_mutex_autolock lock(new_output_lock);
        fprintf(new_output_fp,
                "free: freed %p (size %zu, %zu bytes still allocated)\n",
                usr_ptr, ptr->size, *ptr->context->current_mem_alloc);
    }
#if _DEBUG_NEW_REMEMBER_STACK_TRACE
    free(ptr->stacktrace);
#endif
...
/**
 * Count of source files that use debug_malloc.
 */
...
/**
 * Decrement the count. When the count is zero,
 * nvwa#check_leaks will be called.
 */
void debug_malloc_unuse()
{
    if (--debug_malloc_count == 0 && new_autocheck_flag) {
        if (check_leaks()) {
